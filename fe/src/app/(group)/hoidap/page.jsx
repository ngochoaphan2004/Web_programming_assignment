...
const QuestionPage = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        console.log("Questions fetched:", response.data); // Log dữ liệu câu hỏi
        window.scrollTo(0, 0);
      } catch (error) {
    ...
    const fetchUserContentAndRole = async () => {
      try {
        const [questionsRes, answersRes, userRes] = await Promise.all([
          ...
          axiosConfig.get('user/now', { withCredentials: true })
        ]);
        setUserQuestions(questionsRes.data);
        setUserAnswers(answersRes.data);
        console.log("User data from /user/now:", userRes.data); // Log phản hồi API
        const isAdminStatus = userRes.data.data.role === 1;
        setIsAdmin(isAdminStatus);
        console.log("isAdmin set to:", isAdminStatus); // Log trạng thái isAdmin
      } catch (error) {
  ...
  const handleAskQuestion = async (e) => {
    ...
    try {
      const response = await axiosConfig.post('question/create', {
  ...
  const handleAnswerSubmit = async (questionId) => {
    ...
    try {
      const response = await axiosConfig.post('answer/create', {
  ...
  const handleEditQuestion = async (questionId) => {
    ...
    try {
      const response = await axiosConfig.put(`question/${questionId}`, {
        question_id: questionId,
  ...
  const handleEditAnswer = async (answerId) => {
    ...
    try {
      const response = await axiosConfig.put(`answer/${answerId}`, {
        answer_id: answerId,
...
<h1 className="text-3xl !font-thin mb-6 w-full text-center my-5">HỎI ĐÁP</h1>
        ...
        {questions.length > 0 ? (
          questions.map((q) => (
            ...
            >
              ...
              >
                <span className="mr-2">{showFeedback === q.question_id ? "Ẩn câu trả lời" : "Xem câu trả lời"}</span>
                <svg
              ...
              {showFeedback === q.question_id && (
                <div className="mt-4 text-gray-700 space-y-2">
                  {q.answers?.length > 0 ? (
                    q.answers.map((a) => (
                      <div key={a.id} className="group border p-3 rounded-md bg-gray-50 relative">
              ...
              <div className="mt-6">
                <textarea
                  ...
                  onChange={(e) =>
                    setNewAnswer((prev) => ({ ...prev, [q.question_id]: e.target.value }))
                  }
                ></textarea>
                <button
      ...
      <section className="mt-16">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">Câu hỏi và câu trả lời của bạn</h2>
          <button
        ...
        {showUserContent && (
          <>
            <div className="mb-8">
              <h3 className="text-xl font-semibold mb-4 text-gray-700">Câu hỏi của bạn</h3>
            ...
            <div>
              <h3 className="text-xl font-semibold mb-4 text-gray-700">Câu trả lời của bạn</h3>
                ...
                userAnswers.map((a) => (
                  ...
                  >
                    {editingAnswer?.id === a.id ? (
                      ...
                      <>
                        <div className="mb-4">
                          <h4 className="font-bold text-lg text-gray-800">Câu hỏi: {a.question_title}</h4>
